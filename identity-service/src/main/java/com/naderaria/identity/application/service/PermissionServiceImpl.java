...
import com.naderaria.identity.domain.repository.PermissionRepository;
import com.naderaria.identity.domain.repository.RolePermissionRepository;
...
public class PermissionServiceImpl implements PermissionService {
    ...
    public URI savePermission(ReqPermissionDto reqPermissionDto) {
        ...
        try {
            return WebUtil.createURI(ServletUriComponentsBuilder.fromCurrentContextPath().path("/permission/permission/").toUriString(), permission.getId());
        } catch (Exception e) {
    ...
    private void validateDuplicatePermission(Permission permission) {
        if (permissionRepository.exists(PermissionRepository
            ...
            throw new BusinessException(ErrorCode.DuplicateDataException, "Permission is already exists");
    }
    ...
    private void checkRolePermissionDependOnIt(long permissionId) {
        if (rolePermissionRepository.findByPermissionId(permissionId) > 0)
            throw new BusinessException(ErrorCode.DataReferencedException);
    }
    ...
    public URI saveRole(ReqRoleDto reqRoleDto) {
        ...
        try {
            return WebUtil.createURI(ServletUriComponentsBuilder.fromCurrentContextPath().path("/lab/permission/role/").toUriString() , role.getId());
        } catch (Exception e) {
            throw new BusinessException(ErrorCode.CreateUrlException);
        }
    }
    ...
    private void validateDuplicateRole(Role role) {
        if (roleRepository.exists(RoleRepository.duplicateRole(role.getTitle())))
            throw new BusinessException(ErrorCode.DuplicateDataException, "Role is already exists");
    }
    ...
    public URI saveRolePermission(ReqRolePermissionDto reqRolePermissionDto) {
        ...
        try {
            return WebUtil.createURI(ServletUriComponentsBuilder.fromCurrentContextPath().path("/permission/role/").toUriString() , rolePermission.getId());
        } catch (Exception e) {
            throw new BusinessException(ErrorCode.CreateUrlException);
        }
    }
    ...
    private void validateDuplicateRolePermission(RolePermission rolePermission) {
        if (rolePermissionRepository
                .exists(RolePermissionRepository
                        .duplicateRolePermission(rolePermission.getRole().getId(),
                                rolePermission.getPermission().getId())))
            throw new BusinessException(ErrorCode.DuplicateDataException, "RolePermission is already exists");
    }
